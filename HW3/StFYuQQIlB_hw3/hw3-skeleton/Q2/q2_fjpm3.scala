...
// display(df2)
df2.show(5) // view the first 5 rows of the dataframe
...
// ENTER THE CODE BELOW
//val df3 = df_filter.select("PULocationID")
...
// ENTER THE CODE BELOW
val df_day = df_filter.withColumn("day_of_week",date_format(col("pickup_datetime"), "EEEEEE")).groupBy("day_of_week").agg(avg("PULocationID")).withColumnRenamed("avg(PULocationID)","avg_count")
.withColumn("avg_count",col("avg_count").cast("float")).orderBy(desc("avg_count")) 
//df_day.select("day_of_week","avg_count").show()
df_day.show(2)
