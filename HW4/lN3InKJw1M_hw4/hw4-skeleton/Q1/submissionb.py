...
def dump_results(command, iterations, result):
   ...
   ## with open(command+'_iter'+str(10)+".txt","w") as output_file:
   ##     output_file.write(output_result)



hello = "simplified_pagerank"


pr = PageRank("network.tsv")
pr.calculate_node_degree()
max_node_id = pr.get_max_node_id()
grado_nodos = pr.get_node_degree()

grado_nodos[2]

wen_pr_values = [0.0] * (max_node_id + 1)
par_values = [1.0 / (max_node_id + 1)] * (max_node_id + 1)
node_weights = np.ones(max_node_id + 1) / (max_node_id + 1)
result = pr.run_pagerank(node_weights=node_weights, iterations=5, damping_factor=0.85)
dump_results(hello, 10, result )
