...
def author():                                                                                             
        return "fjpm3" # replace gburdell3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903272633 # replace with your GT ID number      
...
class PageRank:
    ...
    def calculate_node_degree(self):
        ...
        #############################################
            ...
            if (self.max_node_id < source):
                self.max_node_id = source
            if (self.max_node_id < target):
            ...
            if source in self.node_degree:
                ...
                self.node_degree.update({source:1})
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        ### Implement your code here
        #############################################
                part_a = ((1-damping_factor)*node_weights[target]) 
                        
                new_pr_values[target] +=  pr_values[source]/(self.node_degree[source])
                
                pr_values[target] = part_a + damping_factor * new_pr_values[target]
                if pr_values[source] == node_weights[source]:##8.63687e-07:
                    pr_values[source] *= part_a
            pass
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="sample command: python submission.py -i 5 -d 0.85 personalized_pagerank network.tsv")
    parser.add_argument("command",  help="Sub-command to execute. Can be  simplified_pagerank or personalized_pagerank.")
    ##parser.add_argument("command", "simplified_pagerank")
    parser.add_argument("filepath", help="path of the input graph file(network.tsv)")
    ##parser.add_argument("filepath", help="C:/Users/fpuer/Documents/GitHub/C6242-HW/HW4/lN3InKJw1M_hw4/hw4-skeleton/Q1/network.tsv")
